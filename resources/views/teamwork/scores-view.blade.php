@extends('base-template')

@section('title')
    Calificaciones
@stop

@section('head')
    <link href="{!! asset('/css/scores/teamwork.css') !!}" rel="stylesheet">
    <script>
    var questionData = {!!json_encode($questionData)!!};
    teamwork = {!!$teamworks!!};
    arrayScore = {!!$arrayScore!!};
    teamworkid = {!!$teamworkid!!};
</script>
    {{--<script>--}}
    {{--activityGroups = {!! $problem || [] !!};--}}
    {{--</script>--}}
    
@stop

@section('content')

    <h1 class="exercise-name">{!! $exercise !!}</h1>

    <div class="section-selectors">
        <ul>
            <li class="section-selector active" data-target="#scores-individual-section">Calificaciones Individuales</li>
            <li class="section-selector" data-target="#scores-team-section">Calificaciones Grupales</li>
            <li class="section-selector" data-target="#statistics-section">Estadísticas</li>
            <li class="section-selector" data-target="#questions-section">Preguntas</li>
        
        </ul>
    </div>
    <div class="sections-container">

        <section class="scores-section section active" id="scores-individual-section">
        <div class="activities-table-container">
                <table class="table">
                    <thead>
                        <tr>
                            <th>N°</th>
                            <th>Actividad</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($activitiesnoTeam as $index => $activity)
                            <tr>
                                <td class="bold">{!! $index !!}</td>
                                <td>{!! $activity !!}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>

            <div class="scores-table-nogroup-container">
                <table class="datatable table">
                    <thead>
                        <tr>
                            <th>Nombre Alumno</th>
                            @foreach($activitiesnoTeam as $index => $activity)
                                <th>{!! $index !!}</th>
                            @endforeach
                            <th >Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($students as $student)
                        <tr>
                            <td>{!! $student['fullName']!!}</td>
                            @foreach($student['scoreNoTeam'] as $score)
                                <td>{!! $score !!}%</td>
                            @endforeach
                          <td class="bold">{!! $student['exerciseScorenoTeam'] !!}%</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </section>

        <section class="scores-section section" id="scores-team-section">
        <div class="activities-table-container">
                <table class="table">
                    <thead>
                        <tr>
                            <th>N°</th>
                            <th>Actividad</th>
                        </tr>
                    </thead>
                    <tbody>
                    @foreach($activitiesTeam as $index => $activity)
                            <tr>
                                <td class="bold">{!! $index !!}</td>
                                <td>{!! $activity !!}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>

            <div class="scores-table-teamgroup-container">
            @foreach($teamworkss as $twss)
                <table class="datatable table">
                <thead>
                    <tr>
                        <th>{!!$twss['nameTeam']!!}</th>
                    </tr>
                    <tr>
                        <th>Nombre Alumno</th>
                        @foreach($activitiesTeam as $index => $activity)
                            <th>{!! $index !!}</th>
                        @endforeach
                        <th>Total</th>
                    </tr>
                </thead>
                    @foreach($twss['students'] as $student)
                    <tbody>      
                            <tr>
                            @if($student['fullName'] != null)
                                <td>{!! $student['fullName']!!}</td>
                            @endif
                            @if($student['fullName'] == null)
                                <td></td>
                            @endif
                            @foreach($student['scores'] as $score)
                                <td>{!! $score !!}%</td>
                            @endforeach
                            <td class="bold">{!! $student['exerciseScore'] !!}%</td>
                            </tr>
                    </tbody>
                    @endforeach
                    <tr>
                        <th>Total grupo :</th>
                        @foreach($activitiesTeam as $index => $activity)
                            <th></th>
                        @endforeach
                        <th>{!!$twss['teamScore']!!}%</th>
                    </tr>
                </table>
                @endforeach
            </div>
        </section>
        <section class="statistics-section section" id="statistics-section">
            <div class="widgets-container">
                <div class="stat-widget" id="exercise-score-avg">
                    <span class="stat-widget-title">Promedio Curso</span>

                    <div class="stat-widget-content">
                        {!! $statistics['exerciseScoreAvg'] !!}%
                    </div>
                </div>

                <div class="stat-widget">
                    <span class="stat-widget-title">Calificaciones Más Altas</span>

                    <div class="stat-widget-content">
                        <table>
                            <thead>
                                <tr>
                                    <th>Alumno</th>
                                    <th>Calificación</th>
                                </tr>
                            </thead>
                            <tbody>
                                @for($i = 1; $i < 4 && $i <= count($sortedStudents); $i++)
                                    <tr>
                                        <td>{!! $sortedStudents[count($sortedStudents) - $i]['fullName'] !!}</td>
                                        <td>{!! $sortedStudents[count($sortedStudents) - $i]['scoreTotal'] !!}</td>
                                    </tr>
                                @endfor
                            </tbody>
                        </table>
                    </div>
                </div>
                <div class="stat-widget">
                    <span class="stat-widget-title">Calificaciones Más Bajas</span>

                    <div class="stat-widget-content">
                        <table>
                            <thead>
                                <tr>
                                    <th>Alumno</th>
                                    <th>Calificación</th>
                                </tr>
                            </thead>
                            <tbody>
                                @for($i = 0; $i < 3 && $i < count($sortedStudents); $i++)
                                    <tr>
                                        <td>{!! $sortedStudents[$i]['fullName'] !!}</td>
                                        <td>{!! $sortedStudents[$i]['scoreTotal'] !!}</td>
                                    </tr>
                                @endfor
                            </tbody>
                        </table>
                    </div>
                </div>

                <div class="stat-widget">
                    <span class="stat-widget-title">Promedio por Actividad</span>
                    <div class="stat-widget-content">
                        <canvas id="scoresChart" width="900" height="400"></canvas>
                    </div>
                </div>
            </div>
        </section>
        <section id="questions-section" class="questions-section section">
            @include('qualifications.questions')
        </section>
    </div>
    @stop


@section('scripts')

    <script src="{!! asset('/js/vendor/chart.min.js') !!}"></script>
    @section('scripts')
    <script>

        var ctx = document.getElementById('scoresChart');
        var chartData = {
            type : 'bar',
            data : {
                labels : {!! json_encode($activities) !!},
                datasets : [{
                    label : 'Calificación Promedio',
                    data : {!! json_encode($statistics['activitiesScoreAvg']) !!},
                    backgroundColor : [@foreach($activities as $activity) 'rgba(101,1,100, 0.3)', @endforeach],
                    borderColor : [@foreach($activities as $activity) '#650164', @endforeach],
                    borderWidth : 1
                }]
            },
            options : {
                scales : {
                    yAxes : [{
                        ticks : {
                            beginAtZero : true
                        }
                    }]
                }
            }
        };

        var chart = new Chart(ctx, chartData);
        

    </script>
    <script>

        $(document).ready(function() {

            //Maneja el cambio de secciones
            $('.section-selector').on('click',function() {
                var section = $(this).attr('data-target');

                $('.section-selector.active').removeClass('active');
                $(this).addClass('active');

                $('.section.active').removeClass('active');
                $(section).addClass('active');

            })
        });

    </script>
    <script src="{!! asset('/js/analysis/views/question.js') !!}"></script>
<script src="{!! asset('/js/analysis/main.js') !!}"></script>
    



@stop